...
public class GetKdmId extends AbstractProcessor {
	...
	public void onPropertyModified(PropertyDescriptor descriptor, String oldValue, String newValue) {
		...
		if (descriptor.getName().equals("Code_count")) {
			String[] str=newValue.split("_");
			codeCounts=Integer.parseInt(str[1])-Integer.parseInt(str[0]);
			ids=new int[codeCounts];
			for (int i = 0; i < ids.length; i++) {
				ids[i]=i+Integer.parseInt(str[0]);
			}
		}
	}
	...
	public void onTrigger(ProcessContext processContext, ProcessSession processSession) throws ProcessException {
		...
		int back = rdb.readFloatRealDatasByIds(ids, listHolder);
		FloatRealData[] dataLists = listHolder.value;
		
		logger.info("查库时间："+(System.currentTimeMillis()-time2));
		int multiple = codeCounts / batchSize;
		int remainder = codeCounts % batchSize;
		int batch = batchSize;
